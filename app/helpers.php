...
if (!function_exists('extract_role_names')) {
    function extract_role_names(array $roles) {
...
if (!function_exists('extract_permission_names')) {
    function extract_permission_names($role) {
        return $role->permissions->pluck('name','id');
    }
}


if (!function_exists('array_full_diff')) {
    function array_full_diff(array $arr1, array $arr2) {
        return array_merge(array_diff($arr1, $arr2), array_diff($arr2, $arr1));
    }
}

// if (!function_exists('get_403_abstract_error')) {
//     function get_403_abstract_error()
//     {
//         return (new App\Http\Resources\GeneralResponse())
//             ->withCode(Illuminate\Http\Response::HTTP_FORBIDDEN)
//             ->withStatus("error")
//             ->withMessage(trans('auth.forbidden'));
//     }
// }
